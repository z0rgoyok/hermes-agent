...
def _resolve_reasoning(model: str, params: dict[str, Any]) -> tuple[Any, bool]:
    ...
    if params.get("is_xai_responses", False):
        ...
        supported = XAI_GROK46_EFFORTS if is_grok_xhigh_priority_family(model) else XAI_LEGACY_EFFORTS
    else:
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        # Older xAI models reject ``service_tier`` (HTTP 400); Grok 4.6/4.7 accept Priority Processing.
        # Continue stripping stale or unsupported tier values on
        # every other xAI path. See #28490 and #84799.
        if is_xai_responses:
            from agent.model_metadata import is_grok_xhigh_priority_family

            if not (is_grok_xhigh_priority_family(model) and kwargs.get("service_tier") == "priority"):
